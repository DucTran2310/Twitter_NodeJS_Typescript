        ...
        custom: {
          options: async (value, { req }) => {
            if (req.body.type !== TweetTypeEnum.TWEET && !value) {
              throw new Error(TWEET_MESSAGE.PARENT_ID_IS_REQUIRED);
            }
            if (req.body.type !== TweetTypeEnum.TWEET && !ObjectId.isValid(value)) {
              throw new Error(TWEET_MESSAGE.PARENT_ID_CAN_NOT_BE_INVALID);
            }
            if (req.body.type === TweetTypeEnum.TWEET && value) {
            ...
            if (value) {
              ...
              if (
                ...
                throw new Error(TWEET_MESSAGE.TWEET_TYPE_MUST_BE_PUBLIC);
              }
            }
            return true;
          },
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            if (value.some((item: string) => typeof item !== "string")) {
              throw new Error(TWEET_MESSAGE.HASHTAGS_MUST_BE_STRINGS);
            }
            return true;
          },
        },
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            if (value.some((item: string | ObjectId) => !ObjectId.isValid(item))) {
              throw new Error(TWEET_MESSAGE.MENTIONS_MUST_BE_STRINGS);
            }
            return true;
          },
        },
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            if (
              ...
              throw new Error(TWEET_MESSAGE.MEDIAS_MUST_BE_OBJECTS);
            }
            return true;
          },
        },
